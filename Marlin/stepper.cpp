 ...
 */
/**
 梯形是随时间变化的速度曲线。它从初始速率开始，加速
第一个块-加速到步骤结束，然后一直以恒定的速度前进
步骤完成后，在它减速之后，它会减速直到梯形发生器被重置。
加速度的斜率是用v=u+at来计算的，t是到目前为止步骤的累积计时器值。
*/
...
void Stepper::isr() {
  ...
  for (uint8_t i = step_loops; i--;) {
    ...
    // 推进Bresenham算法计数器;如果轴需要一个步骤，就启动一个脉冲
    #define PULSE_START(AXIS) \
      _COUNTER(AXIS) += current_block->steps[_AXIS(AXIS)]; \
      if (_COUNTER(AXIS) > 0) { _APPLY_STEP(AXIS)(!_INVERT_STEP_PIN(AXIS),0); }

    // Stop an active pulse, reset the Bresenham counter, update the position
	// 停止一个活动的脉冲，重置Bresenham计数器，更新位置
     ...
     */
	 /**
	 * 在X步进脉冲的开始和停止之间，估计步进逻辑已经占据的周期数。
	 * 目前，这一数字仅为5个周期左右。
	 * 真正的值可以通过仔细的测试得到
	 * 一旦添加了任何延迟，延迟代码本身的成本可能会从这个值中减去，以获得更准确的延迟。
	 * 在20个周期（1.25秒）下的延迟将是非常准确的，使用NOPs。
	 * 较长的延迟使用循环。分辨率是8个周期。
	 */
     ...
     */
	/**
	* 如果指定了最小脉冲时间，就得到定时器0值。
	*
	* TCNT0有一个8x的预定标器，所以它每8个周期增加一次。
	* 这是16MHz的0.5µs 和20MHz的0.4µs
	* TCNT0的20计数是一个很好的脉冲延迟。
	* 10µs = 160或200个循环。
	*/
  ...
  if (step_events_completed <= (uint32_t)current_block->accelerate_until) {  //加速阶段
  ...
  else if (step_events_completed > (uint32_t)current_block->decelerate_after) { //减速阶段
    uint16_t step_rate;
    ...
    deceleration_time += interval; // 减速时间累加
