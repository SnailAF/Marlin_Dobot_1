...
#include "MarlinConfig.h"
  ...
  #include <math.h>
     ...
     */
    /**
	 *准备一个分段线性移动，用于delta/scara/笛卡尔与UBL和淡出语义。
	 *这计划。缓冲段多次用于小的增量移动。
	 *返回true如果没有移动，如果移动了false（需要currentposition更新）。
	 */

    bool _O2 unified_bed_leveling::prepare_segmented_line_to(const float (&rtarget)[XYZE], const float &feedrate) {


	SERIAL_PROTOCOLLNPGM("prepare_segmented_line_to");
      if (!position_is_reachable(rtarget[X_AXIS], rtarget[Y_AXIS]))  //如果移动到可到达的边界，就会失败 
        return true; //没有移动，所以current_position仍然是准确的
      ...
      const float cartesian_xy_mm = HYPOT(total[X_AXIS], total[Y_AXIS]);  //水平方向xy的总距离
      #if IS_KINEMATIC
        const float seconds = cartesian_xy_mm / feedrate;                                  //按请求速度移动xy距离
        uint16_t segments = lroundf(delta_segments_per_second * seconds),                  //较理想的距离
                 seglimit = lroundf(cartesian_xy_mm * (1.0 / (DELTA_SEGMENT_MIN_LENGTH))); //最小段长度段数
        NOMORE(segments, seglimit);                                                        // 限制最小段长度（更少的段）
      #else
        uint16_t segments = lroundf(cartesian_xy_mm * (1.0 / (DELTA_SEGMENT_MIN_LENGTH))); //笛卡尔固定区段长度
      #endif

      NOLESS(segments, 1);                        // 至少有一个部分
      const float inv_segments = 1.0 / segments;  // divide once, multiply thereafter

      #if IS_SCARA // 将进给速度从mm/s调到度/s
      ...
      // 只有当ubl活动和目标低于z_fade_height时，才计算每段的水平。
      ...
      for(;;) {  // for each mesh cell encountered during the move
        ...
        // for mesh inset area.
        // 计算网格单元不变量，在单元格中保持恒定不变。
		// 注意到细胞索引，如果点在网格网格之外（在meshinset范围内）
		// 在网格中相邻单元的双线性插值仍然有效。
		// 内部循环每次都会退出（因为超出了单元范围），但是会回到循环的顶部
		// 再次找到相同的相邻单元并使用它，对于网格inset区域来说效率较低。
