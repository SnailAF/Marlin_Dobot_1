...
void Planner::calculate_trapezoid_for_block(block_t* const block, const float &entry_factor, const float &exit_factor) {
  uint32_t initial_rate = CEIL(block->nominal_rate * entry_factor), // 计算初始速率
           final_rate = CEIL(block->nominal_rate * exit_factor);    // 计算最终速率(steps per second)
  ...
  // 做加速的步骤+减速步骤是否超过了step_event_count
  // 然后我们不可能达到名义利率，就不会有巡航。
  // 使用intersection_distance()来计算accel/制动时间
  // 在这个block的尽头达到最终的价格。
  // 梯形还是三角形
  if (plateau_steps < 0) {
  ...
  //填充在关键部分中stepper使用的变量
  CRITICAL_SECTION_START; 	//关中断
...
void Planner::_buffer_steps(const int32_t (&target)[XYZE], float fr_mm_s, const uint8_t extruder) {
   ...
   */
   ...
   /**
	* 这部分代码计算了移动的总长度。
	* 对于笛卡尔机器人来说，xaxis是真正的X运动，对于yaxis来说也是一样的。
	* 但对于corexy机器人来说，这是不正确的。“xaxis”和“yaxis”发动机（应该命名为aaxis和baxis）不能用于X和Y的长度，因为A=X+Y和B=X-Y。
  ...
  // 计算这个动作的时间的倒数。由于之前的检查，没有除以0。
  // 例：在120 mm/s时，60毫米的移动需要0.5秒。这就得到了2.0。
  ...
  */
  ...
  if (moves_queued && !UNEAR_ZERO(previous_nominal_speed)) {
    ...
    // 估计在两个连续段的连接处允许的最大速度。
	// 如果允许的最大速度低于入口/出口安全速度的最小值，
	// 然后机器不再滑行，安全进入/出口速度将被使用。
	// 连接速度将在连续的段之间共享。将连接速度限制在最小值上。
	// 选择较小的名义速度。在滑行过程中，不应达到更高的速度。
  ...
  // 最大的入口速度等于前一个块的最大出口速度。
  block->max_entry_speed = vmax_junction;

  // Initialize block entry speed. Compute based on deceleration to user-defined MINIMUM_PLANNER_SPEED.
  //初始化块入口速度。基于减速到用户定义的最小值。
  ...
  // 初始化计划效率标志
  // 设置标志，如果块总是达到最大的连接速度，不管入口/出口速度如何。
  // 如果一个块可以在物体的长度范围内从名义速度降至零，那么当前块
  // 和下一个块结的速度将保证在减速和加速的最大连接速度上保持一定的速度。
  // 这是由于当前块的名义速度限制了当前和下一个最大连接速度。
  // 因此，在逆向和正向的规划者中，相应的块结速度总是处于最大的连接速度，
  // 对于任何速度减少检查，都可能总是被忽略。
...
void Planner::buffer_segment(const float &a, const float &b, const float &c, const float &e, const float &fr_mm_s, const uint8_t extruder) {
  ...
  // 否则它是在_buffer_steps中完成的。
    ...
    if (lin_dist_e) {
      ...
      #endif // PREVENT_COLD_EXTRUSION //预防冷挤出
      #if ENABLED(PREVENT_LENGTHY_EXTRUDE)
  ...
  // 总是把第一个动作分成两个（如果不是引导或探测） 
  if (!blocks_queued()) {

    #define _BETWEEN(A) (position[A##_AXIS] + target[A##_AXIS]) >> 1 //a1 = 4 a2 = 10 between_a = (a1+a2)/2 
