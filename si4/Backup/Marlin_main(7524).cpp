...
uint8_t useAngle = false;   //dobot的角度控制模式
...
void gcode_get_destination() {
#if ENABLED(DOBOT_ARM)
	if(useAngle == true){
		float buf[XYZE];
		LOOP_XYZ(i) {
	    	if (parser.seen(axis_codes[i])) {
	      		const float v = parser.value_axis_units((AxisEnum)i); 
	      		buf[i] = i == E_AXIS ? v : LOGICAL_TO_NATIVE(v, i);
	    	}
	    	else
	      		buf[i] = current_position[i];
		}
			SERIAL_ECHOPAIR(" A_AXIS =", buf[0]); 
			SERIAL_ECHOPAIR(" B_AXIS =", buf[1]);
			SERIAL_ECHOLNPAIR(" C_AXIS =", buf[3]);
		if(relative_mode)
		{
		...
		destination[E_AXIS] = parser.value_axis_units((AxisEnum)E_AXIS) + (axis_relative_modes[E_AXIS] || relative_mode ? current_position[E_AXIS] : 0);
			...
			SERIAL_ECHOPAIR(" B_AXIS+ =", buf[1]);
			SERIAL_ECHOLNPAIR(" C_AXIS+ =", buf[3]);
		forward_kinematics_DOBOT(buf[X_AXIS],buf[Y_AXIS],buf[Z_AXIS]);
		LOOP_XYZ(i){
			destination[i] = cartes[i];
		}
		return;
  }
#endif
  ...
  LOOP_XYZE(i) {
    ...
    SERIAL_ECHOPAIR(" d0=", destination[0]); 
	SERIAL_ECHOPAIR(" d1=", destination[1]);
	SERIAL_ECHOPAIR(" d2=", destination[2]);
	SERIAL_ECHOLNPAIR(" d3=", destination[3]);
  /**
   * 如果使能了DOBOT_ARM并且，开启角度模式，传过来的是角度
  ...
  if (IsRunning() && G0_G1_CONDITION) {
    gcode_get_destination(); //获取命令中XYZE字符后面的值保存在destination[4]和feedrate_mm_s变量
      ...
      if (MIN_AUTORETRACT <= MAX_AUTORETRACT) {
        ...
        // 当M209自动转位启用时，将E-only移动到固件回撤/恢复移动
        if (autoretract_enabled && parser.seen('E') && !(parser.seen('X') || parser.seen('Y') || parser.seen('Z'))) {
          ...
          if (WITHIN(FABS(echange), MIN_AUTORETRACT, MAX_AUTORETRACT) && retracted[active_extruder] == (echange > 0.0)) {
            current_position[E_AXIS] = destination[E_AXIS]; // 隐藏一个基于g1的回撤/从计算中恢复 Hide a G1-based retract/recover from calculations
...
void gcode_G93()
{
	...
	LOOP_XYZ(i) 
	{
		if (parser.seenval(axis_codes[i])) 
		{
			const float l = parser.value_axis_units((AxisEnum)i),
			            v = i == E_AXIS ? l : LOGICAL_TO_NATIVE(l, i),
			            d = v - stepper.get_axis_position_degrees((AxisEnum)i);
			if (!NEAR_ZERO(d))
			{
				if (i == E_AXIS) didE = true; else didXYZ = true;
				delta[i] = v;        // Without workspaces revert to Marlin 1.0 behavior
			}
		}
	}
	...
	report_current_position();
	SERIAL_PROTOCOLLNPGM("gcode_G93 end");
}
// G94 - No Use Angle Coordinates
void gcode_G94()
{
	useAngle = false;
	SERIAL_PROTOCOLLNPGM("useAngle = false");
}
...
{
	useAngle = true;	
	SERIAL_PROTOCOLLNPGM("useAngle = true");
}
#endif
#if HAS_RESUME_CONTINUE
...
void report_current_position() {
 ...
 #if IS_DOBOT
	SERIAL_PROTOCOLLNPAIR(" DOBOT Z :", stepper.get_axis_position_degrees(C_AXIS));
    SERIAL_EOL();
  #endif
  #endif
}
...
void get_cartesian_from_steppers() {
  ...
  #elif ENABLED(DOBOT_ARM)
	forward_kinematics_DOBOT(
		stepper.get_axis_position_degrees(A_AXIS),
		stepper.get_axis_position_degrees(B_AXIS),
		stepper.get_axis_position_degrees(C_AXIS)
	);
  #else
    #if IS_SCARA
...
void set_current_from_steppers_for_axis(const AxisEnum axis) {
   ...
   */
   /**
	*在DELTA或SCARA设置中准备一个线性移动。
	*这个规划.buffer_line几次，为DELTA或SCARA添加小的增量移动。
	*对于统一的床平（Delta或分段笛卡尔坐标）
	* 可以用ubl.prepare_segmented_line_to方法取代。
	*/
  inline bool prepare_kinematic_move_to(const float (&rtarget)[XYZE]) {

  	SERIAL_PROTOCOLLN("in prepare_kinematic_move_to");
    ...
    while (--segments) {
      ...
      static millis_t next_idle_ms = millis() + 200UL;
      thermalManager.manage_heater();  //如果不是真的需要，它会立即返回。
      if (ELAPSED(millis(), next_idle_ms)) {
        next_idle_ms = millis() + 200UL;
        idle();
      }
...
*准备一个动作，为下一个动作做好准备
*
*这可能会导致对计划者的多次调用。buffer_line来
*为DELTA、SCARA、网格移动等进行较小的移动。
*
*确保当前位置E和目标E都很好
*在呼叫或冷/长挤压之前可能会被忽略。
*/
void prepare_move_to_destination() {
	...
	//检查目标位置是否超出限制值，destination是绝对位置值，单位mm
	//暂时关闭CLAMP
  //clamp_to_software_endstops(destination); 
  refresh_cmd_timeout();
  SERIAL_PROTOCOLLN("prepare_move_to_destination");
  SERIAL_ECHOPAIR(" pmtd2_d0=", destination[0]); 
	SERIAL_ECHOPAIR(" d1=", destination[1]);
...
SERIAL_PROTOCOLLN("prepare_move_to_destination end af");
  ...
  void controllerFan() {
   ...
   /** 
    * 由角度得到坐标
    * 算法：http://www.learnaboutrobots.com/inverseKinematics.htm
...
#define lengthRearSquared	SQ((float)lengthRearArm)
#define lengthFrontSquared  SQ((float)lengthFrontArm)

#define armSquaredConst		(lengthRearSquared + lengthFrontSquared)
...
void forward_kinematics_DOBOT(const float &a, const float &b, const float &c)
{
  ...
  /**
   * 由坐标逆向得到角度值。
   * 后支臂垂直于地面，前支臂水平于地面为复位状态。
   * 所以最后有个piHalf减去计算出的角度
...
{	
    ...
    float actualZ,radius,radiusTool,jointX,jointY,hypotenuseSquared,hypotenuse;
	float q1,q2,baseAngle,rearAngle,frontAngle;
...
// end DOBOT_ARM
...
#elif ENABLED(MORGAN_SCARA) && !ENABLED(DOBOT_ARM)
   ...
   */
  /**
   * Morgan SCARA逆运动学。导致delta[].。
   *
   * See http://forums.reprap.org/read.php?185,283327
   *
   * 数学和第一个版本的q哈雷。
   * 集成到Marlin，并由Joachim Cerny进行了稍微重组。
   利用到的公式
   	cosA = (b^2 + c^2 - a^2)/2bc 余弦定理  
   	(cosa)^2 + (sina)^2 = 1
   */
...
void loop() {
  ...
  if (commands_in_queue) {
	...
	//队列可以由命令处理程序重置，也可以由处理器中的idle（）调用的代码重置。
    if (commands_in_queue) {
      --commands_in_queue;
